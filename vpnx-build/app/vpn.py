            ...
            for line in lines[2:]:  # skip header rows
                ...
                if len(parts) >= 15 and parts[14]:
                    self.servers.append({
                        "hostname": parts[0],
                        "ip": parts[1],
                        "score": int(parts[2]) if parts[2].lstrip("-").isdigit() else 0,
                        "ping": int(parts[3]) if parts[3].lstrip("-").isdigit() else 9999,
                        "speed": int(parts[4]) if parts[4].lstrip("-").isdigit() else 0,
                        "country": parts[6],
